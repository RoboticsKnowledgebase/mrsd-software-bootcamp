    ...
    def pose_callback(self, pose_msg):
        ...
        if collision_flag:
            ...
            if len(goal_node.parent) != 0:
                print("FOUND RRT SOLUTION!")
                for idx, node in enumerate(goal_node.parent):
                    path, effort = self.find_path(node)
                    print("PATH #", idx, "PATH: ", path, "EFFORT: ", effort)
                    if effort < min_effort:
                        min_effort = effort
                        min_effort_path = path
                        chosen_idx = idx
                        print("CHOSEN PATH #", chosen_idx)
                # time.sleep(5)
            else:
                print("CANNOT FIND RRT GOAL SO CHOOSING NEAREST")
                path, effort = self.find_path(self.nearest_node_to_goal_node)
                # if effort < min_effort:
                min_effort = effort
                min_effort_path = path
                print("PATH: ", path, "EFFORT: ", effort)
            ...
            if desired_steering_angle > 0.30:
                desired_steering_angle = 0.30
            elif desired_steering_angle < -0.30:
                desired_steering_angle = -0.30
